...
select
  ...
  slope_final,
  case
    when slope_final > 1500 then 'H'
    when slope_final >= 740
    and slope_final <= 1500 then 'M'
    else 'L'
  end as slope_flag
from sen_slope
...
Select
  ...
  tenure,
  case
    when avg_annual_revenue > 3200 then 'H'
    when avg_annual_revenue >= 1330
    and avg_annual_revenue <= 3200 then 'M'
    else 'L'
...
select
  ...
  case
    ...
    or (
      flag_total_clv = 'H'
      and slope_flag = 'H'
      and  (p12m_clv >= 4000 and p12m_clv < 12500
    ))
     then 'Emerging'
    /*** At risk***/
    when 
...
-- MAGIC %py
-- MAGIC spark.table("value_segment").write.parquet(os.path.join(base_dir, f"value_segment{postfix}.parquet"), mode="overwrite")

-- COMMAND ----------

-- MAGIC %py
-- MAGIC spark.table("total_clv").write.parquet(os.path.join(base_dir, f"total_clv{postfix}.parquet"), mode="overwrite")
