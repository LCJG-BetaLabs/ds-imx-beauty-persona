...
from
  (SELECT * FROM AvedaSalesProduct WHERE region_key = "TW")
group by
  year(order_date),
  month(order_date),
  vip_main_no;
...
select
  ...
  slope_final,
  case
    when slope_final > 850 then 'H'
    when slope_final >= 450
    and slope_final <= 850 then 'M'
    else 'L'
  end as slope_flag
from sen_slope
...
Select
  ...
  tenure,
  case
    when avg_annual_revenue > 1500 then 'H'
    when avg_annual_revenue >= 702
    and avg_annual_revenue <= 1500 then 'M'
    else 'L'
...
select
  ...
  case
    ...
    or (
      flag_total_clv = 'H'
      and slope_flag = 'H'
      and  (p12m_clv >= 2000 and p12m_clv < 3800
    ))
     then 'Emerging'
    /*** At risk***/
    when 
...
SELECT * FROM total_clv where inactive_flag = "Inactive P24"

-- COMMAND ----------

-- MAGIC %py
-- MAGIC spark.table("value_segment").write.parquet(os.path.join(base_dir, f"value_segment{postfix}_tw.parquet"), mode="overwrite")

-- COMMAND ----------

-- MAGIC %py
-- MAGIC spark.table("total_clv").write.parquet(os.path.join(base_dir, f"total_clv{postfix}_tw.parquet"), mode="overwrite")
